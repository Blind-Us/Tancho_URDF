...
DEFAULT_TASK = "Template-TanchoV2-Direct-v0"
...
TOOLS = [
    ...
    {
        "name": "start_job",
        "description": "Start an approved zero-agent, training, or policy-play job and return a job id.",
        "inputSchema": {
            "type": "object",
            "required": ["profile"],
            "properties": {
                "profile": {"type": "string", "enum": ["zero_agent", "train", "play"]},
...
def _build_command(args: dict[str, Any]) -> list[str]:
    ...
    defaults = {"zero_agent": 8, "train": 4096, "play": 16}
